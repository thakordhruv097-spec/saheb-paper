...
export const DocumentPrintPreviewModal: React.FC<DocumentPrintPreviewModalProps> = ({
  ...
  const handlePrint = () => {
    ...
    // 2. Trigger prominent download notification toast
    if (onToast) {
        ...
        <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-2.5 border-b border-slate-200 pb-3 pr-8 sm:pr-10 print:hidden">
          ...
          <div className="flex items-center gap-2 shrink-0">
            <button
              ...
              disabled={isViewer}
              onClick={handlePrint}
              title={isViewer ? 'Printing is locked for Viewer (Read-Only Mode)' : 'Print or Save Document as PDF'}
            ...
            <button
              type="button"
              onClick={onClose}
              className="px-3 py-1.5 sm:py-2 text-[10px] sm:text-xs font-bold text-slate-600 hover:bg-slate-100 rounded-xl transition cursor-pointer"
            >
              Close
            </button>
          </div>
